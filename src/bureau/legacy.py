...
class Registry:
    ...
    def validate(self) -> None:
        ...
        commitments = {"now", "next", "later", "waiting", "someday", "completed"}
        task_states = {
            "inbox",
            "planned",
            "ready",
...
class StateStore:
    ...
    def bind(self, run_id: str, system: str, external_id: str) -> dict[str, Any]:
        ...
        with self.immediate() as connection:
            ...
            connection.execute(
                "UPDATE runs SET external_system=?,external_id=?,state='running',updated_at=?,heartbeat_at=? WHERE run_id=?",
    ...
    def claim_next(
        ...
        with self.store.immediate() as connection:
            current = connection.execute(
                "SELECT * FROM runs WHERE worker_id=? AND state IN ('assigned','running','verifying')",
                (worker_id,),
            ).fetchone()
            if current:
                return {
