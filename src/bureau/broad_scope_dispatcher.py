...
_BROAD_SCOPE_REVIEWED_TASK: ContextVar[str | None] = ContextVar(
    "bureau_broad_scope_reviewed_task", default=None
)
...
def _read_stable_regular_file(path: Path) -> tuple[bytes, str]:
    ...
    try:
        ...
        raise legacy.StateError(f"cannot open broad-scope reviewed plan: {exc}") from exc
    try:
        before = os.fstat(descriptor)
        if not stat.S_ISREG(before.st_mode):
            raise legacy.StateError("broad-scope reviewed plan is not a regular file")
        if before.st_size > _MAX_REVIEWED_PLAN_BYTES:
            raise legacy.StateError("broad-scope reviewed plan exceeds the size limit")
        ...
        if first != second:
            raise legacy.StateError("broad-scope reviewed plan changed while it was read")
        if (
            before.st_dev,
            before.st_ino,
            before.st_size,
            ...
            raise legacy.StateError("broad-scope reviewed plan identity changed while it was read")
        try:
        ...
        if (
            ...
            raise legacy.StateError("broad-scope reviewed plan path changed while it was read")
...
def _required_text(value: Any, *, field: str, maximum: int) -> str:
    if not isinstance(value, str) or not value.strip() or len(value) > maximum:
        raise legacy.StateError(f"broad-scope reviewed plan has invalid {field}")
    return value.strip()
...
def _reviewed_plan_evidence(task: legacy.Task) -> dict[str, Any]:
    ...
    if not isinstance(exception, dict):
        raise legacy.StateError("broad Bureau scope exception contract is missing")
    raw_path = exception.get("reviewed_plan_path")
    if not isinstance(raw_path, str) or not raw_path.strip() or len(raw_path) > 4096:
        raise legacy.StateError(
            "broad Bureau scope requires execution.broad_bureau_scope_exception."
            "reviewed_plan_path"
        )
    ...
    try:
        ...
        raise legacy.StateError(f"broad-scope reviewed plan is invalid JSON: {exc}") from exc
    if not isinstance(plan, dict) or plan.get("kind") != "bureau_operator_task_proposal":
        raise legacy.StateError("broad-scope reviewed plan has an unsupported kind")
    if plan.get("command") != "operator-task-propose":
        raise legacy.StateError("broad-scope reviewed plan has an unsupported command")
    ...
    if plan.get("proposal_sha256") != proposal_sha256:
        raise legacy.StateError("broad-scope reviewed plan proposal digest mismatch")
    ...
    reviewer = _required_text(review.get("reviewer"), field="reviewer", maximum=200)
    reviewed_at = _required_text(review.get("reviewed_at"), field="reviewed_at", maximum=80)
    if review.get("reviewed_proposal_sha256") != proposal_sha256:
        raise legacy.StateError("broad-scope reviewed plan review binding mismatch")

    if plan.get("task_id") != task.id:
        raise legacy.StateError("broad-scope reviewed plan task id mismatch")
    task_json = plan.get("task_json")
    if not isinstance(task_json, dict) or task_json != task.raw:
        raise legacy.StateError("broad-scope reviewed plan task payload mismatch")
    if plan.get("task_json_sha256") != task.sha256:
        raise legacy.StateError("broad-scope reviewed plan task digest mismatch")
    if legacy.sha256_json(task_json) != task.sha256:
        raise legacy.StateError("broad-scope reviewed plan current task digest mismatch")
    if plan.get("target_path") != f"registry/tasks/{task.id}.json":
        raise legacy.StateError("broad-scope reviewed plan target path mismatch")
    unresolved = plan.get("unresolved_fields")
    if not isinstance(unresolved, list) or unresolved:
        raise legacy.StateError("broad-scope reviewed plan contains unresolved fields")
    publication = plan.get("publication")
    if not isinstance(publication, dict) or publication.get("action_class") != "registry_mutation":
        raise legacy.StateError("broad-scope reviewed plan publication contract is invalid")
    if publication.get("required_level") != "reviewed_plan":
        raise legacy.StateError("broad-scope reviewed plan required level is invalid")
...
def _broad_scope_assessment(task: legacy.Task, resources: dict[str, Any]) -> dict[str, Any]:
    ...
    def claim_intent(
        ...
        *,
        ...
        if task_id is not None:
            ...
            if task is None:
                raise legacy.StateError(f"unknown coordinated claim task {task_id}")
            assessment = _broad_scope_assessment(task, self.registry.resources)
            if assessment["broad_scope_requested"]:
                if not assessment["allowed"]:
                    raise legacy.StateError("broad Bureau scope exception contract is invalid")
        ...
        if not isinstance(operator_approval, dict):
            raise legacy.StateError("coordinated claim operator approval is invalid")
        operator_approval["broad_scope_review"] = evidence
        intent["intent_sha256"] = coordinated_claim_intent_sha256(intent)
        result["ready_supply"]["broad_scope_review_bound"] = True
        result["ready_supply"]["broad_scope_review"] = {
        ...
        *,
        ...
        with self.store.connect() as connection:
            existing = connection.execute(
                "SELECT 1 FROM runs WHERE run_id=?", (intent["run_id"],)
            ).fetchone()
        ...
        if assessment["broad_scope_requested"]:
            ...
            if supplied != expected:
                raise legacy.StateError(
                    "broad Bureau scope requires matching digest-bound reviewed plan evidence"
                )
            token = _BROAD_SCOPE_REVIEWED_TASK.set(task.id)
        elif supplied is not None:
            raise legacy.StateError("coordinated claim has unexpected broad-scope review evidence")
        try:
            return super().commit_claim_intent(
                intent, lease_binding, resource_db=resource_db
            )
