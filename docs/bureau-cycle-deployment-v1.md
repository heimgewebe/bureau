...
The closure planner alone permits `AF_INET` and `AF_INET6`, because its existing contract observes GitHub pull requests through `gh`; discovery, curator, operator and verifier remain restricted to `AF_UNIX`.
