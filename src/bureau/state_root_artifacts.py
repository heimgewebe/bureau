...
def _require_descriptor_relative_mutation() -> None:
    ...
    if reason is not None:
        raise legacy.StateError(
            "descriptor-relative state-root mutation is unsupported: " + reason
        )
    ...
    try:
        for index, component in enumerate(target.parts[1:]):
            try:
                ...
                if not missing:
                    raise legacy.StateError(
                        "migration destination layout is invalid"
                    ) from None
...
def state_root_migration_plan(
    ...
    *,
    ...
    destination_base, destination_components, destination_base_anchor = (
        _destination_layout(destination)
    )
    with ExitStack() as stack:
        root_descriptor = stack.enter_context(_open_directory_no_follow(root))
        root_parent_descriptor = stack.enter_context(
            _open_directory_no_follow(root.parent)
        )
        reference_descriptor = stack.enter_context(
            _open_directory_no_follow(reference)
        )
        ...
        for name in names:
            ...
            processes = _process_references(anchored_source.resolve(strict=True))
            if references:
                raise legacy.StateError(
        ...
        directory_anchors = {
            ...
            "state_root_parent": _directory_anchor(
                root.parent, root_parent_descriptor
            ),
    ...
    plan = {
        ...
        "execution_preconditions": [
            ...
            (
                "Apply compensates moves from this run and removes its empty "
                "created directories on failure."
            ),
        ],
        "does_not_establish": [
...
def _validate_migration_paths(
    ...
    destination = _absolute_lexical_path(
        Path(str(plan.get("destination_root", "")))
    )
    ...
    destination_base = _absolute_lexical_path(
        Path(str(layout.get("base_path", "")))
    )
    ...
    if (
        ...
        or any(
            ...
            or _MIGRATION_ENTRY_RE.fullmatch(component) is None
    ...
    _validate_anchor_shape(
        anchors.get("state_root"), expected_path=root, role="state-root"
    )
    _validate_anchor_shape(
    ...
    return {
        "state_root": stack.enter_context(
            _open_bound_directory(
                anchors["state_root"], expected_path=root, role="state-root"
            )
        ),
    ...
    try:
        for component in components:
            if _lstat_at(current_descriptor, component) is not None:
                raise legacy.StateError(
                    "migration destination collision: component appeared since "
                    f"review: {current_path / component}"
                )
            try:
    ...
    for record in reversed(created):
        try:
            ...
            if (
                ...
                raise legacy.StateError(
                    "created destination path no longer names its bound "
    ...
    for item in reversed(moved):
        ...
        try:
            ...
            if (
                ...
                and _entry_identity(
                    _descriptor_path(destination_descriptor, destination_name)
                )["entry_sha256"]
                == item["entry_sha256"]
            ):
                _rename_at(
...
def _recover_failed_effect(
    *,
    ...
    if failures:
        raise legacy.StateError(
            f"state-root mutation failed with incomplete recovery: {cause}; "
            + "; ".join(failures)
        ) from cause
...
def apply_state_root_migration_plan(path: Path) -> dict[str, Any]:
    ...
    with ExitStack() as stack:
        ...
        if unexpected:
            raise legacy.StateError(
                "new migration destination unexpectedly contains entries: "
                + ", ".join(unexpected)
            )
        ...
        try:
            ...
            for expected in planned_entries:
                ...
                if source_info is None or stat.S_ISLNK(source_info.st_mode):
                    raise legacy.StateError(
                        "migration source is missing or symlinked: "
                        f"{root / source_name}"
                    )
                ...
                processes = _process_references(
                    _descriptor_path(descriptors["state_root"], source_name).resolve(
                        strict=True
                    )
                )
            ...
            for item in prepared:
                ...
                _assert_plan_directory_paths_current(descriptors, anchors)
                _assert_created_destination_paths_current(created)
                source_info = _lstat_at(
                    descriptors["state_root"], item["source_name"]
                )
                if source_info is None or stat.S_ISLNK(source_info.st_mode):
                    raise legacy.StateError(
                ...
                if references or processes:
                    raise legacy.StateError(
                        f"migration source gained references: {item['source']}"
                    )
                _require_same_filesystem(source_info, destination_descriptor)
                _rename_at(
                ...
                if (
                    _entry_identity(
                        _descriptor_path(
                            destination_descriptor, item["destination_name"]
                        )
                    )["entry_sha256"]
                    != item["entry_sha256"]
                ):
...
def rollback_state_root_migration(receipt_path: Path) -> dict[str, Any]:
    ...
    destination_root = _absolute_lexical_path(
        Path(str(receipt.get("destination_root", "")))
    )
    reference_root = _absolute_lexical_path(
        Path(str(receipt.get("reference_root", "")))
    )
    ...
    with ExitStack() as stack:
        ...
        for action in actions:
            ...
            destination_info = _lstat_at(
                descriptors["state_root"], destination_name
            )
            if destination_info is not None:
                if (
                    ...
                    continue
                raise legacy.StateError(
                    f"rollback destination collision: {destination_key}"
                )
            if source_info is None or stat.S_ISLNK(source_info.st_mode):
                raise legacy.StateError(
                    f"rollback source is missing or symlinked: {source_key}"
                )
            if (
                ...
                raise legacy.StateError(
                    f"rollback source identity mismatch: {source_key}"
                )
            references = _reference_hits(
                _descriptor_path(descriptors["reference_root"]),
                Path(source_key),
            ...
            processes = _process_references(
                _descriptor_path(destination_descriptor, source_name).resolve(
                    strict=True
                )
            )
            if references or processes:
                raise legacy.StateError(
                    f"rollback source gained references: {source_key}"
                )
        ...
        try:
            for item in planned:
                ...
                source_info = _lstat_at(
                    destination_descriptor, item["source_name"]
                )
                if source_info is None or stat.S_ISLNK(source_info.st_mode):
                    raise legacy.StateError(
                ...
                if (
                    _lstat_at(
                        descriptors["state_root"], item["destination_name"]
                    )
                    is not None
                ):
                    raise legacy.StateError(
