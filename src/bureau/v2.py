...
def _task_write_scope_for_repository(
    ...
    for index, value in enumerate(resources):
        ...
        try:
            relative = normalized.relative_to(repository_root)
        except ValueError:
            continue
        ...
        try:
            paths.add(_canonical_repo_relative_path(relative_text, label=f"grabowski path {index}"))
        except OpenPullRequestObservationError as exc:
...
def _task_open_pr_scope_assessment(
    ...
    for reservation in reservations:
        ...
        repository_conflicts = any(
            legacy.overlaps(claim.resource, reservation.resource, registry.resources)
            and legacy.modes_conflict(claim.mode, reservation.mode)
            for claim in task.claims
        )
        if not repository_conflicts:
            continue
        task_scope = scope_cache.setdefault(
            reservation.resource,
            _task_write_scope_for_repository(task, registry.resources.get(reservation.resource)),
        )
        ...
        elif task_scope["classification"] != "scope-ready":
            classification = "scope-required"
            diagnostic = str(task_scope["diagnostic"])
        else:
    ...
    def reasons(
        ...
        *,
        ...
        for reservation in open_pr_reservations:
            ...
            if not repository_conflicts:
                continue
            if projection_resource is not None and not legacy.overlaps(
                reservation.resource, projection_resource, self.registry.resources
            ):
                continue
