...
def classify_task_attention(
    ...
    *,
    ...
    def task_success_keys(row: sqlite3.Row) -> set[str]:
        keys: set[str] = set()
        row_keys = set(row.keys())
        # Older Grabowski task databases keep resource leases outside the task row.
        # CWD and argv hash are the stable in-row supersession keys here.
        if "cwd" in row_keys and row["cwd"]:
            keys.add("cwd:" + str(row["cwd"]))
        if "argv_sha256" in row_keys and row["argv_sha256"]:
            keys.add("argv:" + str(row["argv_sha256"]))
        return keys

    newer_success_by_key: dict[str, int] = {}
    for row in rows:
        if str(row["state"]) != "completed":
            continue
        updated = int(row["updated_at_unix"])
        for key in task_success_keys(row):
            newer_success_by_key[key] = max(newer_success_by_key.get(key, 0), updated)
    ...
    for row in rows:
        ...
        if state == "running":
            ...
            if age <= horizon_seconds:
                failed_updated = int(row["updated_at_unix"])
                keys = task_success_keys(row)
                superseded = any(newer_success_by_key.get(key, 0) > failed_updated for key in keys)
                group = "superseded_recent_failed" if superseded else "recent_failed"
            else:
                group = "historical_failed"
        else:
    ...
    return {
        ...
        "interpretation": {
            ...
            "superseded_recent_failed": (
                "recent failed records with a newer successful task on the same resource; "
                "diagnostic history, not current attention"
            ),
        },
    }
