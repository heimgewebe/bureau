...
def test_installed_activation_candidate_validator_rejects_noncanonical_manifest(
    ...
    with pytest.raises(refresh.RuntimeRefreshError) as raised:
        refresh._validate_installed_runtime_activation_candidate(
            ...
            approval_task_id="BUREAU-RUNTIME-NONCANONICAL-MANIFEST",
            candidate={},
        )
    assert (
        raised.value.code
...
def installed_activation_candidate_validation_for_test(
    *,
    ...
    receipt = {
        ...
        "installed_source_commit": expected_deployed_source_commit,
        "deployment_manifest_sha256": "a" * 64,
        "package_tree_sha256": "b" * 64,
        "runtime_module_sha256": "c" * 64,
        "criterion_ids": sorted(
            candidate["metadata"]["runtime_refresh_authority"]
            ["no_run_closeout_acceptance"]["criteria"]
        ),
    }
    return refresh.bind_digest(receipt, "validation_sha256")


def activate_runtime_refresh_authority_for_test(**kwargs: Any) -> dict[str, Any]:
    kwargs.setdefault(
        "installed_candidate_validator",
        installed_activation_candidate_validation_for_test,
    )
    return refresh.activate_runtime_refresh_authority(**kwargs)
...
def seed_protected_publication_activation_store(
    ...
    *,
    ...
    persist_activation_evidence: bool = True,
) -> StateStore:
    ...
    observation = activation_observation or protected_publication_activation_observation()
    ...
    if (
        ...
        activation_evidence = (
            ...
            if persist_activation_evidence
            else None
        )
...
def protected_publication_activation_target_sha256(
    ...
    proof = protected_publication_proof(store, task_id)
    key = (
        f"runtime-refresh-protected-publication-activation:{task_id}:"
        f"{'4' * 40}:{proof['task_spec_sha256']}"
    )
    receipt = store.task_spec_mutation_receipt(key)
    assert isinstance(receipt, dict)
    evidence = receipt.get("activation_evidence")
    assert isinstance(evidence, dict)
    refresh.verify_digest(evidence, "evidence_sha256")
...
def protected_publication_activation_evidence_for(
    *,
    task_id: str,
...
def test_protected_publication_activation_specialized_cas_requires_runtime_observation(
    ...
    state_root = (tmp_path / "missing-observation").resolve()
    store = StateStore(state_root / "bureau.sqlite3", state_root)
    planned = protected_publication_activation_spec(task_id, state="planned")
    adopted = store.put_task_spec(
        planned,
...
def test_protected_publication_activation_accepts_exact_legacy_receipt_without_observation(
    ...
    monkeypatch.setattr(task_specs.legacy, "utc_now", lambda: "2026-09-01T21:00:00Z")
...
def test_protected_publication_activation_rejects_post_cutoff_ready_without_witness(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    task_id = "BUREAU-RUNTIME-PUBLICATION-ACTIVATION-POST-CUTOFF-NO-WITNESS"
    state_root = (tmp_path / "post-cutoff-no-witness").resolve()
    store = StateStore(state_root / "bureau.sqlite3", state_root)
    planned = protected_publication_activation_spec(task_id, state="planned")
    adopted = store.put_task_spec(
        planned,
    ...
    monkeypatch.setattr(task_specs.legacy, "utc_now", lambda: "2026-09-02T00:01:00Z")
    with store.immediate() as connection:
        task_specs._put_validated_material(
            connection,
            task_specs._canonical_spec(ready),
...
def test_protected_publication_activation_witness_is_create_only(
    tmp_path: Path,
) -> None:
    task_id = "BUREAU-RUNTIME-PUBLICATION-ACTIVATION-WITNESS-COLLISION"
    state_root = (tmp_path / "witness-collision").resolve()
    store = StateStore(state_root / "bureau.sqlite3", state_root)
    planned = protected_publication_activation_spec(task_id, state="planned")
    adopted = store.put_task_spec(
        planned,
    ...
    first_observation = protected_publication_activation_observation()
    first_evidence = protected_publication_activation_evidence_for(
    ...
    first = store.put_runtime_refresh_protected_publication_activation_task_spec(
        ...
        idempotency_key=key,
        expected_revision=adopted["revision"],
        activation_observation=first_observation,
        activation_evidence=first_evidence,
    )
    assert first["revision"] == 2
    first_receipt = store.task_spec_mutation_receipt(key)
    assert isinstance(first_receipt, dict)
    assert first_receipt["activation_evidence"] == first_evidence

    second_observation = protected_publication_activation_observation(
        deployed_source_commit="9" * 40
    )
    second_evidence = protected_publication_activation_evidence_for(
        task_id=task_id, adopted=adopted, ready=ready, key=key,
    ...
    with pytest.raises(legacy.StateError) as raised:
        store.put_runtime_refresh_protected_publication_activation_task_spec(
            ...
            idempotency_key=key,
            expected_revision=adopted["revision"],
            activation_observation=second_observation,
            activation_evidence=second_evidence,
        )
    assert "replay evidence differs" in str(raised.value)
    replay_receipt = store.task_spec_mutation_receipt(key)
    assert isinstance(replay_receipt, dict)
    assert replay_receipt["activation_evidence"] == first_evidence
...
def test_protected_publication_activation_rejects_spoofed_source_without_bound_receipt(
    ...
    with pytest.raises(refresh.RuntimeRefreshError) as raised:
        refresh.validate_authoritative_runtime_refresh_task(
            store=store,
            approval_task_id=task_id,
            target_sha256=protected_publication_activation_observation()["target_sha256"],
            target_main_commit=MAIN,
        )
    assert (
        raised.value.code
...
def test_protected_publication_activation_rejects_weakened_contract_delta(
    ...
    evidence = refresh._protected_publication_activation_evidence(
        ...
        adoption_revision=adopted["revision"],
        adoption_spec_sha256=adopted["spec_sha256"],
        activation_spec_sha256=task_specs.task_spec_digest(weakened),
        idempotency_key=key,
        publication_pr=2222,
        publication_merge_commit="4" * 40,
        target_main_commit=MAIN,
...
def test_activation_evidence_rejects_installed_manifest_mismatch() -> None:
    task_id = "BUREAU-RUNTIME-ACTIVATION-MANIFEST-MISMATCH"
    ready = protected_publication_activation_spec(task_id, state="ready")
    observation = protected_publication_activation_observation()
    installed_validation = installed_activation_candidate_validation_for_test(
        manifest_path=Path("/test/deployment-manifest.json"),
    ...
    payload["deployment_manifest_sha256"] = "d" * 64
    mismatched_validation = refresh.bind_digest(payload, "validation_sha256")
    from bureau import task_specs
    ...
    with pytest.raises(refresh.RuntimeRefreshError) as raised:
        refresh._protected_publication_activation_evidence(
            ...
            task_file_sha256="c" * 64,
            observation=observation,
            installed_runtime_validation=mismatched_validation,
        )
    assert (
        raised.value.code
        == "authority-preflight-publication-activation-evidence-invalid"
...
def test_activation_cas_rejects_rebound_manifest_mismatch(
    tmp_path: Path,
) -> None:
    task_id = "BUREAU-RUNTIME-ACTIVATION-MANIFEST-CAS-MISMATCH"
    state_root = (tmp_path / "manifest-cas-mismatch").resolve()
    store = StateStore(state_root / "bureau.sqlite3", state_root)
    planned = protected_publication_activation_spec(task_id, state="planned")
    adopted = store.put_task_spec(
        planned,
...
def test_activate_runtime_refresh_authority_binds_fresh_observation_and_replays(
    ...
    assert (
        ...
        not in current["spec"]["metadata"]
    )
    expected_ready = refresh._expected_protected_publication_activation_spec(
        historical_spec=planned,
        publication={
            "schema_version": refresh.RUNTIME_AUTHORITY_SCHEMA_VERSION,
            "repository": refresh.DEFAULT_REPOSITORY,
            "publication_pr": 2222,
    ...
    assert current["spec"] == expected_ready
    activation_key = (
        f"runtime-refresh-protected-publication-activation:{task_id}:"
        f"{'4' * 40}:{planned_sha}"
    )
    ...
    assert evidence["task_file_sha256"] == "c" * 64
    assert (
        evidence["installed_runtime_validation"]["deployment_manifest_sha256"]
        == observation["deployed_manifest_sha256"]
    )
    assert first["activation_evidence_sha256"] == evidence["evidence_sha256"]
    assert receipt["activation_evidence_sha256"] == evidence["evidence_sha256"]
    assert receipt["resulting_revision"] == 2

    second = activate_runtime_refresh_authority_for_test(
        registry_root=tmp_path / "registry",
        manifest_path=tmp_path / "deployment-manifest.json",
        repository=refresh.DEFAULT_REPOSITORY,
        approval_task_id=task_id,
    ...
    with pytest.raises(refresh.RuntimeRefreshError) as wrong_digest:
        activate_runtime_refresh_authority_for_test(
            registry_root=tmp_path / "unusable-registry-on-replay",
            manifest_path=tmp_path / "deployment-manifest.json",
            repository=refresh.DEFAULT_REPOSITORY,
            approval_task_id=task_id,
            publication_pr=2222,
            ...
            authority_store=store,
            registry=None,
            observer=lambda **_: pytest.fail("wrong-digest replay must not observe"),
        )
    assert (
        wrong_digest.value.code
        == "authority-preflight-publication-activation-evidence-invalid"
    )
    after_wrong_digest = store.task_spec(task_id)
    assert isinstance(after_wrong_digest, dict)
    assert after_wrong_digest["revision"] == 2
...
def test_activate_runtime_refresh_authority_replays_after_late_target_binding(
    ...
    replay = activate_runtime_refresh_authority_for_test(
        registry_root=tmp_path / "registry",
        manifest_path=tmp_path / "deployment-manifest.json",
        repository=refresh.DEFAULT_REPOSITORY,
        approval_task_id=task_id,
...
def test_activation_cas_delayed_replay_checks_receipt_before_freshness(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    clock = {"value": datetime(2026, 9, 2, 15, 0, tzinfo=timezone.utc)}
    monkeypatch.setattr(
        task_specs.legacy, "utc_now", lambda: refresh.isoformat(clock["value"])
    )
    observation = protected_publication_activation_observation(
...
def test_protected_publication_activation_specialized_cas_keeps_immediate_transaction(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    task_id = "BUREAU-RUNTIME-ACTIVATION-CAS-LOCK"
    state_root = (tmp_path / "activation-cas-lock").resolve()
    store = StateStore(state_root / "bureau.sqlite3", state_root)
    planned = protected_publication_activation_spec(task_id, state="planned")
    adopted = store.put_task_spec(
        planned,
    ...
    original_put = task_specs._put_validated_material
    transaction_states: list[bool] = []

    def put_with_transaction_probe(connection: Any, *args: Any, **kwargs: Any) -> dict[str, Any]:
        transaction_states.append(bool(connection.in_transaction))
        return original_put(connection, *args, **kwargs)

    monkeypatch.setattr(task_specs, "_put_validated_material", put_with_transaction_probe)
...
def test_activate_runtime_refresh_authority_validates_installed_candidate_before_cas(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    task_id = "BUREAU-RUNTIME-ACTIVATE-INSTALLED-PRE-CAS"
    state_root = (tmp_path / "activate-installed-pre-cas").resolve()
    store = StateStore(state_root / "bureau.sqlite3", state_root)
    planned = protected_publication_activation_spec(task_id, state="planned")
    from bureau import task_specs
    ...
    observation = protected_publication_activation_observation()
    events: list[str] = []
    installed_receipt: dict[str, Any] = {}

    def installed_validator(**kwargs: Any) -> dict[str, Any]:
        events.append("installed-validator")
        receipt = installed_activation_candidate_validation_for_test(**kwargs)
        installed_receipt.update(receipt)
        return receipt

    original_put = refresh._put_authority_task

    def put_after_installed_validation(*args: Any, **kwargs: Any) -> dict[str, Any]:
        assert events == ["installed-validator"]
        events.append("cas")
        return original_put(*args, **kwargs)

    monkeypatch.setattr(refresh, "_put_authority_task", put_after_installed_validation)
    result = refresh.activate_runtime_refresh_authority(
        ...
        registry=registry,
        observer=lambda **_: observation,
        installed_candidate_validator=installed_validator,
    )
    assert events == ["installed-validator", "cas"]
    key = (
        f"runtime-refresh-protected-publication-activation:{task_id}:"
        f"{'4' * 40}:{planned_sha}"
    )
    ...
    evidence = receipt.get("activation_evidence")
    assert isinstance(evidence, dict)
    assert evidence["installed_runtime_validation"] == installed_receipt
    assert evidence["installed_runtime_validation_sha256"] == installed_receipt[
        "validation_sha256"
    ]
...
def test_runtime_prefix_execution_context_preflight_accepts_grabowski_task_executor(
    ...
    assert evidence["executor_matches_expected_unit"] is True
...
def test_cached_no_effect_replay_rejects_different_consuming_executor(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    _observed, manifest_path, intent, intent_path = prepare_candidate_intent(tmp_path)
    store = authority_store_for_intent(intent)
    ...
    replay_executor = "grabowski-task-replay-a2.service"
    lease_binding, resource_db = lease_for(
        tmp_path / "cross-executor-replay-leases",
        intent,
        metadata={"executor_unit": replay_executor},
    )
    lease_binding["task_id"] = intent["approval_task_id"]
    monkeypatch.setattr(
        refresh,
        "_current_systemd_execution_identity",
        lambda: (replay_executor, "2" * 32),
    )
    ...
    assert caught.value.code == "authority-consumption-execution-context-drift"
    current = store.task_spec(intent["approval_task_id"])
    assert current is not None
    authority = current["spec"]["metadata"]["runtime_refresh_authority"]
    assert authority.get("consumption") is None
...
def test_apply_strict_executor_preserves_lease_task_id_semantics(
    ...
    monkeypatch.setattr(
        refresh,
        "_current_systemd_execution_identity",
        lambda: (executor_unit, "e" * 32),
    )

    def source_preparer(**kwargs: Any) -> dict[str, Any]:
        kwargs["workspace"].mkdir(parents=True)
        return {"head": MAIN, "root": str(kwargs["workspace"]), "dirty": False, "detached": True}
    ...
    assert result["status"] == "deployed"
    assert result["lease_binding"]["task_id"] == intent["approval_task_id"]
    evidence = result["execution_context_preflight"]
    assert evidence["systemd_unit"] == executor_unit
    assert evidence["expected_grabowski_task_unit"] == executor_unit
    assert evidence["executor_matches_expected_unit"] is True
...
def test_apply_executor_unit_drift_blocks_before_authority_cas(
    ...
    lease_binding["task_id"] = intent["approval_task_id"]
    identities = iter([(first_unit, "f" * 32), (second_unit, "1" * 32)])
    monkeypatch.setattr(
        refresh, "_current_systemd_execution_identity", lambda: next(identities)
    )
    store = authority_store_for_intent(intent)
...
def unused_authority_equivalent_success_case(
    tmp_path: Path,
    *,
    ...
    unused_task_id = "BUREAU-RUNTIME-UNUSED-AUTHORITY"
    equivalent_task_id = "BUREAU-RUNTIME-EQUIVALENT-SUCCESS"
    state_root = (tmp_path / "state").resolve()
    bureau_state_root = (tmp_path / "bureau-state").resolve()
    store = StateStore(bureau_state_root / "bureau.sqlite3", bureau_state_root)
    for task_id in (unused_task_id, equivalent_task_id):
        store.put_task_spec(
            runtime_authority_spec(task_id),
            idempotency_key=f"seed-unused-case:{task_id}",
            expected_revision=None,
            source="test",
        )
    selected_now = (
        NOW if older_unused_main_commit is None else NOW + timedelta(minutes=2)
    )
    if older_unused_main_commit is not None:
        older_observed, _older_manifest = candidate(
            tmp_path / "older-unused", main_commit=older_unused_main_commit
        )
        older_common = {
            "candidate": older_observed,
            "state_root": state_root,
            "prefix": (tmp_path / "older-unused/prefix").resolve(),
            "bin_dir": (tmp_path / "older-unused/bin").resolve(),
            "user_unit_dir": (tmp_path / "older-unused/systemd/user").resolve(),
            "libexec_dir": (tmp_path / "older-unused/libexec").resolve(),
            "remote_url": "file:///tmp/bureau.git",
            "authorized_by": "chatgpt",
            "authorization": "Hermetic older unused-authority intent.",
            "break_glass": True,
            "approval_reference": older_observed["target_sha256"],
            "now": NOW,
            "authority_store": store,
        }
        refresh.prepare_intent(
            **older_common, approval_task_id=unused_task_id
        )
    observed, _manifest_path = candidate(tmp_path)
    common = {
        "candidate": observed,
        ...
        "remote_url": "file:///tmp/bureau.git",
        "authorized_by": "chatgpt",
        "authorization": "Hermetic unused-authority closeout fixture.",
        "break_glass": True,
        "approval_reference": observed["target_sha256"],
        "now": selected_now,
        "authority_store": store,
    ...
    install_receipt, historical_readback, _artifacts = historical_runtime_artifacts(
        tmp_path, equivalent_intent
    )
    lease_binding_seed, resource_db = lease_for(
        tmp_path / "unused-closeout-leases",
        equivalent_intent,
        current=selected_now,
    )
    lease_binding = refresh.validate_live_lease_binding(
        equivalent_intent,
        lease_binding_seed,
        resource_db=resource_db,
        now=selected_now,
    ...
    release_test_leases(resource_db)
    assert lease_binding["resource_db"] == str(resource_db.resolve())
    attempt_dir = state_root / "attempts" / equivalent_intent["target_sha256"]
    result = refresh._write_attempt_result(
        attempt_dir / "result.json",
        {
            "schema_version": refresh.SCHEMA_VERSION,
            "kind": "bureau_runtime_refresh_result",
            "status": "deployed",
            "intent_sha256": equivalent_intent["intent_sha256"],
            "target_sha256": equivalent_intent["target_sha256"],
            "main_commit": equivalent_intent["main_commit"],
            "source_identity": {"head": equivalent_intent["main_commit"]},
            "install_receipt": install_receipt,
            "readback": historical_readback,
            "authority_task_spec": bound,
            "lease_binding": lease_binding,
            "finished_at": refresh.isoformat(selected_now + timedelta(minutes=2)),
            "effect_started": True,
            "does_not_establish": ["future_runtime_health"],
        },
    )
    assert result["lease_binding"]["resource_db"] == str(resource_db.resolve())
    persisted_result = refresh.read_json(attempt_dir / "result.json")
    assert persisted_result["lease_binding"]["resource_db"] == str(resource_db.resolve())
    refresh.consume_runtime_refresh_authority(
        store=store,
        intent=equivalent_intent,
        result=result,
        now=selected_now + timedelta(minutes=2),
    )
    revise_authority(
        store,
        equivalent_task_id,
        lambda spec: spec.__setitem__("state", "verified"),
        key="equivalent-success-terminal",
    )
    unused_current = store.task_spec(unused_task_id)
    assert unused_current is not None
    return (
        unused_task_id,
        unused_intent,
        equivalent_task_id,
...
def test_unused_authority_closeout_supersedes_without_runtime_effect(
    ...
    assert before["spec"]["metadata"]["runtime_refresh_authority"].get("consumption") is None
    assert before["spec"]["metadata"]["runtime_refresh_authority"].get(
    ...
    assert closeout["closeout"]["fulfilled_by_task_id"] == equivalent_task_id
    assert closeout["closeout"]["target_sha256"] == unused_intent["target_sha256"]
    assert closeout["closeout"]["current_runtime_source_commit"] == "9" * 40
    current = store.task_spec(unused_task_id)
    assert current is not None
    assert current["spec"]["state"] == "superseded"
    authority = current["spec"]["metadata"]["runtime_refresh_authority"]
    assert authority.get("consumption") is None
    assert authority.get("target_binding_receipt") is None
    replay = store.replay_projection()
    assert replay["matches_current"] is True

    repeated = refresh.closeout_unused_runtime_refresh_authority(
        state_root=Path(unused_intent["state_root"]),
        approval_task_id=unused_task_id,
        expected_revision=before["revision"],
        expected_spec_sha256=before["spec_sha256"],
...
def test_unused_authority_closeout_replay_requires_reserved_mutation_receipt(
    ...
    with store.immediate() as connection:
        connection.execute(
            "DELETE FROM task_spec_mutations WHERE idempotency_key=?", (key,)
        )
...
def test_unused_authority_closeout_holds_resource_db_barrier_through_cas(
    ...
    monkeypatch.setattr(refresh, "_put_authority_task", guarded_put)
    closeout = refresh.closeout_unused_runtime_refresh_authority(
        state_root=Path(unused_intent["state_root"]),
        approval_task_id=unused_task_id,
        expected_revision=before["revision"],
...
def test_unused_authority_closeout_rechecks_runtime_reservation_inside_cas(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    (
        unused_task_id,
    ...
    def racing_reservations(connection: sqlite3.Connection):
        ...
        if calls["count"] >= 3:
            return [
                legacy.Reservation(
                    "BUR-RUN-RACE", "component.bureau.runtime", "write", 1
                )
            ]
...
def captured_unused_authority_closeout_candidate(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> tuple[str, StateStore, dict[str, Any]]:
    (
        unused_task_id,
        unused_intent,
        _equivalent_task_id,
    ...
    class CapturedCloseout(RuntimeError):
        pass

    def capture_put(
        _store: Any, candidate_spec: dict[str, Any], **kwargs: Any
    ) -> dict[str, Any]:
        captured["spec"] = json.loads(json.dumps(candidate_spec))
        captured.update(kwargs)
        raise CapturedCloseout

    monkeypatch.setattr(refresh, "_put_authority_task", capture_put)
    with pytest.raises(CapturedCloseout):
        refresh.closeout_unused_runtime_refresh_authority(
            state_root=Path(unused_intent["state_root"]),
            approval_task_id=unused_task_id,
            expected_revision=before["revision"],
...
def test_state_store_unused_authority_closeout_has_no_independent_mutation_authority(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
...
def test_unused_authority_closeout_rejects_other_resource_database(
    ...
    before = store.task_spec(unused_task_id)
    assert before is not None
    _fake_binding, fake_resource_db = lease_for(
        tmp_path / "other-unused-closeout-leases", equivalent_intent, current=NOW
    )
    release_test_leases(fake_resource_db)
    assert fake_resource_db.resolve() != resource_db.resolve()
...
def test_unused_authority_closeout_uses_actual_time_for_intent_expiry(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    (
        unused_task_id,
    ...
    before = store.task_spec(unused_task_id)
    assert before is not None
    monkeypatch.setattr(refresh, "utc_now", lambda: NOW + timedelta(minutes=5))
...
def test_unused_authority_closeout_accepts_linear_target_ratchet(
    ...
    (
        ...
        tmp_path, older_unused_main_commit=older_main
    )
    before = store.task_spec(unused_task_id)
    assert before is not None
...
def test_unused_authority_closeout_uses_intent_bound_state_store(tmp_path: Path) -> None:
    (
        unused_task_id,
        unused_intent,
        _equivalent_task_id,
    ...
    assert closeout["closeout"]["status"] == "superseded"
    assert store.task_spec(unused_task_id)["spec"]["state"] == "superseded"
    assert not (Path(unused_intent["state_root"]) / "bureau.sqlite3").exists()


def test_unused_authority_closeout_rejects_unverified_equivalent_authority(
    tmp_path: Path,
) -> None:
    (
        unused_task_id,
    ...
    revise_authority(
        ...
        equivalent_task_id,
        lambda spec: spec.__setitem__("state", "superseded"),
        key="equivalent-authority-not-verified",
    )
    current = store.task_spec(unused_task_id)
    assert current is not None
...
def test_unused_authority_closeout_rejects_live_runtime_reservation(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    (
        unused_task_id,
...
def test_unused_authority_closeout_rejects_own_attempt_or_result(tmp_path: Path) -> None:
    (
        unused_task_id,
        unused_intent,
        _equivalent_task_id,
    ...
    attempt_dir = Path(unused_intent["state_root"]) / "attempts" / unused_intent["target_sha256"]
    result_path = attempt_dir / "result.json"
    result_path.unlink()
    own_result = dict(result)
    own_result.pop("result_sha256", None)
    own_result["intent_sha256"] = unused_intent["intent_sha256"]
    refresh._write_attempt_result(result_path, own_result)
    current = store.task_spec(unused_task_id)
    assert current is not None
    with pytest.raises(refresh.RuntimeRefreshError) as caught:
        refresh.closeout_unused_runtime_refresh_authority(
...
def test_unused_authority_closeout_rejects_missing_equivalent_consumption(
    tmp_path: Path,
) -> None:
    (
        unused_task_id,
    ...
    revise_authority(
        ...
        key="remove-equivalent-consumption",
    )
    current = store.task_spec(unused_task_id)
    assert current is not None
    with pytest.raises(refresh.RuntimeRefreshError) as caught:
...
def test_unused_authority_closeout_rejects_live_foreign_lease(tmp_path: Path) -> None:
    (
        unused_task_id,
        unused_intent,
        _equivalent_task_id,
    ...
    connection = sqlite3.connect(resource_db)
    metadata_json = json.dumps(
        {"executor_unit": TEST_EXECUTOR_UNIT},
        sort_keys=True,
        separators=(",", ":"),
    )
    metadata_sha256 = hashlib.sha256(metadata_json.encode("utf-8")).hexdigest()
    acquired = int((NOW + timedelta(minutes=45)).timestamp())
    expires = int((NOW + timedelta(hours=2)).timestamp())
    connection.execute(
        "INSERT INTO leases VALUES (?, ?, ?, ?, ?, ?, ?, ?, NULL)",
        (
            unused_intent["required_resource_keys"][0],
...
def test_unused_authority_closeout_rejects_task_spec_drift(tmp_path: Path) -> None:
    (
        unused_task_id,
        unused_intent,
        _equivalent_task_id,
    ...
    revise_authority(
        store,
        unused_task_id,
        lambda spec: spec.__setitem__("title", "Concurrent drift"),
        key="unused-authority-drift",
    )
    with pytest.raises(refresh.RuntimeRefreshError) as caught:
        refresh.closeout_unused_runtime_refresh_authority(
            state_root=Path(unused_intent["state_root"]),
...
def test_unused_authority_closeout_rejects_projection_replay_divergence(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    (
        unused_task_id,
    ...
    monkeypatch.setattr(
        store,
        "replay_projection",
        lambda: {**original(), "matches_current": False},
    )
    with pytest.raises(refresh.RuntimeRefreshError) as caught:
        refresh.closeout_unused_runtime_refresh_authority(
            state_root=Path(unused_intent["state_root"]),
