...
def _write_reviewed_plan(path: Path, task: dict) -> dict:
    proposal: dict = {
        "schema_version": 1,
...
def _prepare_broad_task(root: Path, plan_path: Path) -> str:
    ...
    task["execution"]["broad_bureau_scope_exception"] = {
        "justification": "One atomic repository-format migration covers all tracked Bureau files.",
...
def _dispatcher(root: Path, state_path: Path) -> tuple[StateStore, Dispatcher]:
...
def test_legacy_claim_surface_cannot_bypass_broad_scope_review(
    ...
    task_id = _prepare_broad_task(root, tmp_path / "reviewed-plan.json")
    store, dispatcher = _dispatcher(root, tmp_path / "state.sqlite3")

    with pytest.raises(NoEligibleTask, match="digest-bound reviewed plan evidence"):
...
def test_coordinated_claim_binds_reviewed_plan_before_lease_validation(
    ...
    assert evidence["proposal_sha256"] == json.loads(
...
def test_operator_only_intent_cannot_impersonate_broad_scope_review(
    ...
    intent = dispatcher.claim_intent(
        "operator", ("repository",), task_id=task_id, approved=True
    )["intent"]
    del intent["operator_approval"]["broad_scope_review"]
    intent["intent_sha256"] = coordinated_claim_intent_sha256(intent)

    with pytest.raises(StateError, match="matching digest-bound reviewed plan evidence"):
        dispatcher.commit_claim_intent(intent, None)
