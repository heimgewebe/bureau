...
def test_inventory_covers_systemd_exec_start_commands() -> None:
    ...
    if snapshot_service.is_file():
        assert (
            units["ops/systemd/bureau-state-snapshot.service"]["matched_console_script"]
            == "bureau"
        )
    assert (
    ...
    assert units["ops/systemd/bureau-task-supply.service"]["command_basename"] == "bureau"
    assert (
        units["ops/systemd/bureau-task-supply.service"]["matched_console_script"]
        == "bureau"
    )
    assert (
